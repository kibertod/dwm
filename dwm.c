...
void resizeclient(Client* c, int x, int y, int w, int h) {
        ...
        while (running) {
            ...
            for (int i = 0; i < event_count; i++) {
                ...
                if (event_fd == dpy_fd) {
                    ...
                    if (ipc_handle_client_epoll_event(events + i, mons,
                            &lastselmon, selmon, tags, LENGTH(tags), layouts,
                            LENGTH(layouts)) < 0) {
        ...
        for (i = my = ty = 0, c = nexttiled(m->clients); c;
            ...
            if (i < m->nmaster) {
                ...
                resize(c, m->wx, m->wy + my,
                    mw - (2 * c->bw) + (n > 1 ? gappx : 0), h - (2 * c->bw), 0);
                if (my + HEIGHT(c) < m->wh)
                    my += HEIGHT(c);
            } else {
                h = (m->wh - ty) / (n - i);
    ...
    void unmanage(Client * c, int destroyed) {
        ...
        if (!destroyed) {
            ...
            XConfigureWindow(
                dpy, c->win, CWBorderWidth, &wc); /* restore border */
            XUngrabButton(dpy, AnyButton, AnyModifier, c->win);
